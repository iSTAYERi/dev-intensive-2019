...
class ChatAdapter(val listener: (ChatItem) -> Unit) : RecyclerView.Adapter<ChatAdapter.ChatItemViewHolder>() {
    ...
    inner class SingleViewHolder(override val containerView: View) : TouchableChatItemViewHolder(containerView){
        override fun bind(item:ChatItem, listener: (ChatItem) -> Unit){
            if (item.avatar==null) {
                Glide.with(itemView)
                    .clear(containerView.iv_avatar_single)
                containerView.iv_avatar_single.setImageDrawable(containerView.resources.getDrawable(R.color.color_accent, containerView.context.theme))
                containerView.iv_avatar_single.setText(item.initials)
            } else {
                containerView.iv_avatar_single.setText(null)
                Glide.with(itemView)
                    .load(item.avatar)
                    .into(containerView.iv_avatar_single)

            }

            containerView.sv_indicator.visibility = if (item.isOnline) View.VISIBLE else View.GONE

            with(containerView.tv_date_single){
                visibility = if (item.lastMessageDate!=null) View.VISIBLE else View.GONE
                text = item.lastMessageDate
            }

            with(containerView.tv_counter_single){
                visibility = if (item.messageCount>0) View.VISIBLE else View.GONE
                text = item.messageCount.toString()
            }

            containerView.tv_title_single.text = item.title
            containerView.tv_message_single.text = item.shortDescription
    ...
    inner class GroupViewHolder(override val containerView: View) : TouchableChatItemViewHolder(containerView){
        override fun bind(item: ChatItem, listener: (ChatItem) -> Unit) {
            containerView.iv_avatar_group.setText(item.title[0].toString())
            //sv_indicator.visibility = if (item.isOnline) View.VISIBLE else View.GONE

            with(containerView.tv_date_group){
                visibility = if (item.lastMessageDate!=null) View.VISIBLE else View.GONE
                text = item.lastMessageDate
            }

            with(containerView.tv_counter_group){
                visibility = if (item.messageCount>0) View.VISIBLE else View.GONE
                text = item.messageCount.toString()
            }

            containerView.tv_title_group.text = item.title
            containerView.tv_message_group.text = item.shortDescription

            with(containerView.tv_message_author){
                visibility = if (item.messageCount>0) View.VISIBLE else View.GONE
                text = "@"+item.author
            }
            itemView.setOnClickListener{
    ...
    inner class ArhiveViewHolder(override val containerView: View): ChatItemViewHolder(containerView){
        override fun bind(item: ChatItem, listener: (ChatItem) -> Unit) {
            with(containerView.tv_date_archive){
                visibility = if (item.lastMessageDate!=null) View.VISIBLE else View.GONE
                text = item.lastMessageDate
            }

            with(containerView.tv_counter_archive){
                visibility = if (item.messageCount>0) View.VISIBLE else View.GONE
                text = item.messageCount.toString()
            }

            containerView.tv_message_archive.text = item.shortDescription

            with(containerView.tv_message_author_archive){
                visibility = if (item.messageCount>0) View.VISIBLE else View.GONE
                text = "@"+item.author
            }
            itemView.setOnClickListener{
